...
import { Wand2, FlaskConical, Sparkles, FileText, BookOpen, Palette, Layout, RefreshCw, Tag, RotateCcw, Zap, Layers, Brain, User, Menu, X } from 'lucide-react';
...
export const PromptLab: React.FC<PromptLabProps> = ({ isSidebarOpen = false }) => {
    ...
    const [isSecondarySidebarOpen, setIsSecondarySidebarOpen] = useState(window.innerWidth >= 1024);
    ...
    // Close secondary sidebar on mobile when menu item selected
    ...
    const ComponentsActions = (
        <div className="flex items-center gap-5">
            <label className="flex items-center gap-2.5 cursor-pointer select-none group bg-purple-50 px-3 py-1.5 rounded-lg border border-purple-100 transition-all hover:bg-purple-100 hover:border-purple-200 hidden md:flex">
    ...
    const renderContent = () => {
        switch (selectedMenu) {
            ...
            case 'components':
                return (
                    <div className="px-2 md:px-10 pb-6 pt-4 space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
    ...
    const HeaderButtons = (
        <div className="flex items-center gap-3">
            <Button
                ...
                className="hover:bg-red-50 hover:text-red-600 hover:border-red-300 hidden md:flex"
            >
                Reset
            </Button>
            <Button
    ...
    return (
        ...
        >
            <div className="flex h-full relative overflow-hidden">
                ...
                {/* Secondary Sidebar */}
                <div className={clsx(
                    "w-64 h-full flex-shrink-0 flex flex-col bg-white border-r border-slate-200 transition-all duration-300 z-30",
                    "absolute lg:static",
                    isSecondarySidebarOpen ? "translate-x-0 shadow-xl lg:shadow-none" : "-translate-x-full lg:translate-x-0"
                )}>
                    {/* Mobile Close Button */}
                    <div className="lg:hidden p-2 flex justify-end">
                        <button onClick={() => setIsSecondarySidebarOpen(false)} className="p-2 text-slate-500">
                            <X size={20} />
                        </button>
                    </div>

                    <div className="p-2 pt-2 lg:pt-12 space-y-2 flex-1 overflow-y-auto custom-scrollbar">
                ...
                <div className="flex-1 h-full flex flex-col min-w-0 bg-slate-50 overflow-hidden">
                    ...
                    <div className="shrink-0 h-20 bg-white border-b border-slate-200 px-4 flex items-center justify-between sticky top-[24px] z-10">
                        <div className="flex items-center gap-4">
                            ...
                            <h2 className="text-sm font-bold text-slate-800 uppercase tracking-wider hidden md:block">
                                {headerInfo.title}
                            </h2>
                            <div className="h-4 w-px bg-slate-200 hidden md:block"></div>
                            <p className="text-xs text-slate-400 truncate max-w-[150px] md:max-w-none">{headerInfo.subtitle}</p>
                        </div>
