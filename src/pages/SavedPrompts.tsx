...
import { Search, Trash2, Clock, BookOpen, X, FileText, FolderOpen, Edit, Cpu, Download, Brain, Sparkles, LayoutGrid, Orbit, List } from 'lucide-react';
import { promptDB, SavedPrompt } from '@/services/database';
import { vectorDb } from '@/services/vectorDbService';
import { PromptGalaxy } from '@/components/galaxy/PromptGalaxy';
...
export const SavedPromptsLibrary: React.FC<SavedPromptsLibraryPropsExtended> = ({ isSidebarOpen = false, onNavigate }) => {
    ...
    const SearchBar = (
        <div className="flex items-center gap-2">
            {/* View Mode Toggle */}
            <div className="flex items-center p-1 bg-slate-100 rounded-lg mr-2 border border-slate-200">
                <button
                ...
                <button
                    ...
                    className={`p-1.5 rounded-md transition-all ${viewMode === 'galaxy' ? 'bg-slate-900 text-cyan-400 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                    title="Galaxy View (3D)"
                >
                    <Orbit size={16} />
                </button>
            </div>
        ...
        <>
            ...
            >
                <div className="h-full overflow-y-auto bg-slate-50 p-6">
                    {/* Content Area */}
                    <div className={`flex-1 overflow-y-auto p-4 md:p-8 min-h-0 bg-slate-50 relative ${viewMode === 'galaxy' ? '!p-0' : ''}`}>
                        {savedPrompts.length === 0 ? (
                            ...
                            <>
                                {viewMode === 'galaxy' && (
                                    <div className="w-full h-[600px] animate-in fade-in zoom-in-95 duration-500 rounded-xl overflow-hidden border border-slate-200 shadow-xl bg-black relative z-10">
                                        <PromptGalaxy
                                            onNodeClick={(id) => {
                                                const pid = id.replace('prompt-', '');
                                                const p = allPrompts.find(x => x.id.toString() === pid);
